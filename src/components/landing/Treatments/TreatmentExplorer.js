...
export default function TreatmentExplorer({ categories }) {
  ...
  const [activeTreatmentId, setActiveTreatmentId] = useState(categories[0]?.treatments[0]?.id ?? null);
  const panelRef = useRef(null);

  if (categories.length === 0) return null;

  const activeCategory = categories.find((category) => category.id === activeCategoryId);
  const activeTreatments = activeCategory?.treatments ?? [];
  const activeTreatment = activeTreatments.find((treatment) => treatment.id === activeTreatmentId) || activeTreatments[0];

  const imageSrc = activeTreatment?.imageUrl || null;
  const imageAlt = activeTreatment?.imageAlt || activeTreatment?.title;

  function handleCategoryChange(categoryId) {
    if (categoryId === activeCategoryId) return;
    const nextCategory = categories.find((category) => category.id === categoryId);
    setActiveCategoryId(categoryId);
    setActiveTreatmentId(nextCategory?.treatments[0]?.id ?? null);
  }
  ...
  return (
    <div className={styles.explorer}>
      ...
      <div className={styles.layout}>
            ...
            <AnimatePresence mode="wait">
              ...
              >
                {activeTreatment ? (
                  ...
                  <p className={styles.panelText}>Nenhum tratamento cadastrado nesta categoria.</p>
                )}
              </motion.div>
            </AnimatePresence>
          </div>
        ...
        <StaggerGroup key={activeCategory?.id ?? 'empty'} className={styles.list} as="ul">
